...
const updateFileFavorite = async (id, favorite) => {
  return axios.post(`https://patient-portal-ixd8.onrender.com/documents/${id}/favorite`, { favorite });
};
...
export const FileProvider = ({ children }) => {
  ...
  const fetchFiles = async () => {
    try {
      const response = await axios.get('https://patient-portal-ixd8.onrender.com/documents');
  ...
  const uploadFile = async (file) => {
    ...
    try {
      const response = await axios.post('https://patient-portal-ixd8.onrender.com/documents/upload', formData, {
  ...
  const deleteFile = async (id) => {
    try {
      await axios.delete(`https://patient-portal-ixd8.onrender.com/documents/${id}`);
  ...
  const downloadFile = (id) => {
    window.open(`https://patient-portal-ixd8.onrender.com/documents/${id}/download`, '_blank');
  };
